...
import com.fusai.lib.db.TASQLiteDatabase.TADBParams;
import com.fusai.lib.db.TASQLiteDatabase.TADBUpdateListener;
import com.fusai.lib.log.TALogger;

/**
 * @description 数据库管理池
 * @author 许友爻
 * @date 2014年6月17日 下午6:53:59
 * @version 1.0
 */
public class TASQLiteDatabasePool
{
	 ...
	 */
	public synchronized static TASQLiteDatabasePool getInstance(Context context, TADBParams params, Boolean isWrite)
	{
	 ...
	 *            打开数据库如果是 isWrite为true,则磁盘满时抛出错误
	 */
	public static TASQLiteDatabasePool getInstance(Context context, String dbName, int dbVersion, Boolean isWrite)
	{
	 ...
	 *            打开数据库如果是 isWrite为true,则磁盘满时抛出错误
	 */
	public TASQLiteDatabasePool(Context context, TADBParams params, Boolean isWrite)
	{
	...
	private void createSQLiteDatabase(int numSQLiteDatabase)
	{
		...
		for (int x = 0; x < numSQLiteDatabase; x++)
		{
			...
			{
				TALogger.i(TASQLiteDatabasePool.this, " 创建数据库连接失败！ " + e.getMessage());
			}
	...
	public synchronized TASQLiteDatabase getSQLiteDatabase()
	{
		...
		{
			TALogger.i(TASQLiteDatabasePool.this, "没有可用的数据库连接，等一会儿再试");
			// 等一会再试
			wait(250);
