package com.fastandroid.lib.http.AsyncHttpClient;
...
import com.fastandroid.exception.FileAlreadyExistException;
...
public class FileHttpResponseHandler extends AsyncHttpResponseHandler
{
	...
	{
		sendMessage(obtainMessage(PROGRESS_MESSAGE, new Object[] { totalSize, currentSize, speed }));
	}
	...
	protected void sendResponseMessage(HttpResponse response)
	{
		...
		try
		{
			...
			{
				TALogger.v(TAG, "Output file already exists. Skipping download.");
			...
			else if (tempFile.exists())
			{
				...
				// +
				// "-");
				TALogger.v(TAG, "yahooo: " + response.getEntity().getContentLength());
				previousFileSize = tempFile.length();

				TALogger.v(TAG, "File is not complete, download now.");
				TALogger.v(TAG, "File length:" + tempFile.length() + " totalSize:" + totalSize);
